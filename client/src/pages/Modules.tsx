...
// Древовидная группировка категорий с точным соответствием требованиям (все 22 категории)
const categoryTree = {
  "ВСЕ МОДУЛИ": [],
  ...
  "ОТРАСЛЕВЫЕ РЕШЕНИЯ": ["ОТРАСЛЕВЫЕ РЕШЕНИЯ"], 
  "АНАЛИТИКА": ["АНАЛИТИКА"], 
  "СОЦИАЛЬНАЯ КОММЕРЦИЯ": ["СОЦИАЛЬНАЯ КОММЕРЦИЯ"], 
  "AI И АВТОМАТИЗАЦИЯ": ["AI И АВТОМАТИЗАЦИЯ"], 
  "AI-АВАТАРЫ": ["AI-АВАТАРЫ"], 
  "ПАРСИНГ TELEGRAM": ["ПАРСИНГ TELEGRAM"],
                ...
                {keyFeatures.map((feature, index) => (
                  <div key={index} className="flex items-start gap-3">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed break-words hyphens-auto">
                      {formatFeatureText(feature).replace(/^\*\*|\*\*$/g, '').replace(/\*\*/g, '')}
                    </p>
                  </div>
                ))}
              </div>
