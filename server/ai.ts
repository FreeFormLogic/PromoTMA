...
function calculateModuleRelevance(module: any, analysis: BusinessAnalysis): number {
  ...
  if (analysis.industry === 'tourism') {
    // Для турагентств - точная приоритизация с отраслевым модулем
    if (module.number === 261) score += 500; // Отраслевой модуль для турагентств - абсолютный приоритет
  ...
  // Универсально полезные модули (кроме случаев, когда есть отраслевой модуль)
  if (analysis.industry === 'tourism' && module.number === 224) {
    score += 20; // Меньше баллов для платежей при наличии отраслевого модуля
  } else if ([1, 224, 13, 8].includes(module.number)) {
    score += 50;
  }
...
export async function generateAIResponse(messages: { role: 'user' | 'assistant'; content: string }[], alreadyShownModules: number[] = []): Promise<{ response: string; recommendedModules: number[] }> {
  try {
    ...
    const moduleExplanations = recommendedModules.map(moduleNum => {
      ...
      switch (analysis.industry) {
        case 'tourism':
          if (moduleNum === 261) return `[MODULE:261] Комплексная система управления турагентством с интеграцией туроператоров, поиском туров и CRM для туристов.`;
