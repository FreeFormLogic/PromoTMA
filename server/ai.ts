...
export function calculateModuleRelevance(
  ...
  // Специальные бонусы для туризма
  if (analysis.industry.toLowerCase().includes('туризм') || 
      analysis.industry.toLowerCase().includes('туристический') ||
      analysis.keywords.some(k => ['тур', 'отель', 'экскурсия', 'путешествие', 'турвизор'].includes(k.toLowerCase()))) {
    
    // Туристические модули с высокими приоритетами
    const tourismModules: Record<number, number> = {
      // КРИТИЧЕСКИ ВАЖНЫЕ для туризма
      78: 80,   // CRM - управление клиентской базой туристов
      112: 90,  // Бронирование - основа туристического бизнеса  
      152: 70,  // Интеграции 1С - может интегрироваться с Турвизор
      143: 75,  // Календарь и бронирование - основа туризма
      13: 65,   // Программа лояльности - повторные клиенты
      
      // ОЧЕНЬ ВАЖНЫЕ
      116: 60,  // Личный кабинет клиента - если есть в базе
      151: 80,  // Интеграции без программирования - если есть в базе
      70: 55,   // Международный эквайринг - иностранные туристы
      120: 60,  // GoPay/OVO - для работы с Бали/Индонезией
      69: 45,   // Платежные системы
      
      // ВАЖНЫЕ ДОПОЛНЕНИЯ
      146: 40,  // Мультиязычность - для иностранцев
      162: 35,  // AI-чат 24/7 - поддержка туристов
      17: 30,   // Аналитика - анализ продаж туров
      169: 45,  // Отель - если есть размещение
      173: 35,  // Логистика - трансферы и доставка
      31: 40,   // Уведомления - важно для туристов
      
      // СПЕЦИФИЧЕСКИЕ ДЛЯ ТУРИЗМА
      104: 25,  // Медиагалерея - фото туров
      148: 30,  // Календарь событий - расписание экскурсий  
      140: 35   // AI-поддержка - консультации по турам
    };
    
    if (tourismModules[module.number]) {
      score += tourismModules[module.number];
    }
  }
...
**Если ТУРИЗМ/ТУРИСТИЧЕСКОЕ АГЕНТСТВО**:
- [MODULE:112] Система бронирования туров
- [MODULE:151] Интеграции с Турвизор  
- [MODULE:116] Личный кабинет туриста
- [MODULE:78] CRM клиентов
