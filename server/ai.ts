...
async function getRecommendationsForChunk(
  ...
  const systemPrompt = `Ты — элитный бизнес-архитектор. Проанализируй диалог с клиентом и, опираясь на предоставленный ЧАСТИЧНЫЙ СПИСОК модулей, выбери из него 2-3 САМЫХ подходящих решения. Удели особое внимание отраслевым модулям, если они есть в списке и подходят бизнесу.
...
ПРИОРИТЕТ ВЫБОРА:
1.  **Отраслевые решения:** Если в списке есть модуль, название которого точно совпадает с бизнесом клиента (например, "Управление турагентством" для турагентства) — выбери его в первую очередь.
2.  **Ключевые бизнес-процессы:** Затем выбери модули, которые закрывают основные потребности (бронирование, платежи, каталог).
3.  **Рост и маркетинг:** В конце добавь модули для привлечения клиентов и повышения лояльности.

СПИСОК ЛУЧШШИХ МОДУЛЕЙ-КАНДИДАТОВ:
---
${finalModuleDatabase}
---
...
2.  Следуя ПРИОРИТЕТУ ВЫБОРА, выбери из списка кандидатов 4-5 САМЫХ лучших модуля.
