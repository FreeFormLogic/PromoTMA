...
export async function generateAIResponse(messages: { role: 'user' | 'assistant'; content: string }[], alreadyShownModules: number[] = []): Promise<{ response: string; recommendedModules: number[] }> {
  try {
    ...
    const systemPrompt = `Ты эксперт по Telegram Mini Apps. У тебя 259 модулей для разных бизнес-задач.
...
СПЕЦИАЛЬНЫЕ ПРАВИЛА:
- ПИЦЦЕРИЯ/РЕСТОРАН: модуль 165 (управление рестораном), 177 (BI-дашборды), 196 (видеоконференции)
- БАЛИ/ИНДОНЕЗИЯ: модули 120, 123, 125 (локальные платежи)
- E-COMMERCE: модули 1-50 (базовые), 208 (динамическое ценообразование), 207 (предиктивные запасы)
- CRM: модули 78, 111 (основные), 176 (платформа клиентских данных)
- ТУРАГЕНТСТВО: модули 13, 78, 111, 161 (основные), 177 (BI-аналитика), 179 (анализ отзывов), 204 (UGC), 209 (AI-генератор контента)
- САЛОН КРАСОТЫ: модули 147, 13, 156 (основные), 179 (анализ отзывов), 196 (видеоконференции)
- АНАЛИТИКА/ДАННЫЕ: модули 176, 177, 178, 179, 181, 182, 183, 184
- AI И АВТОМАТИЗАЦИЯ: модули 206, 207, 208, 209, 210, 211, 213, 214, 215
- БЕЗОПАСНОСТЬ: модули 186, 191, 192
- КОММУНИКАЦИИ: модули 196, 199
- МАРКЕТИНГ: модули 204, 209, 213, 217
