...
export async function generateAIResponse(messages: { role: 'user' | 'assistant'; content: string }[], alreadyShownModules: number[] = []): Promise<{ response: string; recommendedModules: number[] }> {
  try {
    ...
    const systemPrompt = `Ты эксперт по Telegram Mini Apps. У тебя 260 модулей для разных бизнес-задач.
...
СПЕЦИАЛЬНЫЕ ПРАВИЛА:
- ОТЕЛИ/ГОСТИНИЦЫ: модули 112 (умная запись), 113 (групповые бронирования), 114 (столики в ресторане), 120/123/125 (платежи для Индонезии)
- ПИЦЦЕРИЯ/РЕСТОРАН: модуль 165 (управление рестораном) первым, 114 (бронирование столиков)
- БАЛИ/ИНДОНЕЗИЯ: модули 120, 123, 125 (локальные платежи) ОБЯЗАТЕЛЬНО
- E-COMMERCE: модули 1-50 (товары, корзина, платежи)
- CRM: модули 78, 111 (клиентская база, интеграции)
- ТУРАГЕНТСТВО: модули 13 (лояльность), 78 (клиентская база), 111 (интеграции), 161 (push-уведомления)
- САЛОН КРАСОТЫ: модули 112 (умная запись), 13 (лояльность), 156 (управление расписанием)
